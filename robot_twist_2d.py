...
class Robot:
	...
	def draw(self, canvas):
		axis_lines = (self.T@self.axis_lines.T).astype("int").T
		cv2.line(canvas, axis_lines[0, :2], axis_lines[1, :2], (255, 0, 0), 1)
		cv2.line(canvas, axis_lines[2, :2], axis_lines[3, :2], (0, 0, 255), 1)
	...
	def rotate(self, angle, degrees = False):
		...
		self.T = np.append(R, self.o.T, axis = 1)
		self.T = np.append(self.T, np.array([[0,0,1]]), axis=0)
		positions = body_IK(x = 0, y = 60, body_length = self.w, angle = angle)
...
origin_x = W//2
origin_Y = H//2
robot = Robot(origin_x, origin_Y, 0)

ang = 0
while True:
	canvas = (np.ones((W, H, 3))*255).astype("uint8")
	cv2.line(canvas, (0, origin_Y+60), (W, origin_Y+60), (0, 0, 0), 2)
	cv2.circle(canvas, (origin_x-50, origin_Y+60), 10, (255,255,0),-1)
	cv2.circle(canvas, (origin_x+50, origin_Y+60), 10, (255,255,0),-1)
	robot.draw(canvas)
